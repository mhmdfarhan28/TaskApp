...
fun EditTaskScreen(
    ...
    val targetTask by viewModel.targetTasks.collectAsState(initial = 0)
    val task =  viewModel.getTaskById(targetTask)
    ...
    LaunchedEffect(Unit){
        viewModel.getTaskById(targetTask)
        viewModel.setTitleScreen("Edit task")
    }
        ...
        Column(
            ...
            Row(
                ...
                Button(
                    onClick = {
                        MainScope().launch {
                            viewModel.editTask(targetTask, title!!, description!! , completed!!)
